...
1. If data is missing from the grid table, this message will appear. This occurs when the
   elevation raster, roughness polygon, or roughness raster does not fully cover the grid.
   Check the grid attributes for NULL values.

   .. image:: ../../img/Export-Project/exportproject4.png

2. If a component switch is off, an action is taken to correct the switch position using the
   Export dialog box. The following short video demonstrates how the export action works.

   .. raw:: html

      <iframe width="560" height="315"
              src="https://www.youtube.com/embed/iVAZcKqD2_w?si=UNr-bT5iegXnyufZ"
...
3. Export warning
   ...
   .. warning::
      If you commonly export to the same folder, the **Remove FPLAIN.DAT, CADPTS.DAT, and
      NEIGHBOR.DAT** option may be active. Enable this option if changes were made to grid
      shape, size, elevation, or roughness. Otherwise, the FLO-2D engine will read existing
      files.
