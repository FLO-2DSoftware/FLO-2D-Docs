...
that highlight FLO-2D model project applications.
...
Channel infiltration is based solely on hydraulic conductivity and represents average bed and bank seepage conditions.
...
The flood volume controls the area of inundation (Figure 129).
...
It is possible to simulate rainfall while routing a flood overland and have the rainfall occur directly to the water surface without abstraction.
...
Basin water course depressions created in the grid elevation because of the lack of DTM data on the channel bed can be smoothed out with the GDS
smoothing tool.
...
The FLO-2D storm drain component is fully integrated with the surface water system including channels and detention basins (Figure 131).
...
Figure 132 shows velocity vector and flow depth results in an urban area with the integrated storm drain system.
...
There are numerous webinars and Power Point presentations on project review and troubleshooting at the FLO-2D website.
...
bridge.
The component enables the user to compute the bridge discharge without using an external program to generate a stagedischarge rating curve or table as
required in the past for the FLO-2D model.
Three classes of flow regime are computed: Free surface flow for discharge below the bridge low chord;

Pressure flow when the discharge is above the low chord but below the bridge deck; and Combined pressure and weir flow as the discharge goes over the
bridge.
...
Ultimately, bridge flow control with coefficient adjustments, however, should provide a suitable correlation between the models.
...
In this manner, the flow can be assessed as one-dimensional with no variation in water surface elevation in the bridge channel cross-section.
...
blockage.
Sluice gate flow may persist until the upstream flow depth is 1.5 times or greater than the depth to the low chord.
...
This is defined as drowned orifice flow and can only happen when both upstream and downstream water surface elevations exceed 1.1 times the height of
the bridge opening.
...
The HEC-RAS computed water surfaces elevations are shown in Figure 139.
...
Los Lunas Bridge are shown in Figure 139.
Pressure flow begins at about 18,000 cfs.
Figure 140 and Figure 141 indicate that the FLO2D predicted stage upstream of the bridge at and below the low chord matches the HEC-RAS stage exactly.
...
because the discharge overtops the levees and flows beyond the cross-sections both upstream and downstream (Figure 144).
...
The FLO-2D channel outflow node pulls down the in-channel water surface, but much of the water is also overbank on the floodplain at that location.
...
For higher discharges, the FLO-2D model under predicted the HEC-RAS stages because the floodplain area of inundation was not filled.
...
downstream as shown in Figure 124.
